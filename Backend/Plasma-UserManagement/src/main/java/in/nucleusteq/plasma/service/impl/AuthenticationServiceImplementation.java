...
public class AuthenticationServiceImplementation implements AuthenticationService {
	...
	public LoginOutDTO loginUser(LoginInDTO inputUserDto, HttpServletResponse httpServletResponse) throws TokenServiceException {
		...
		if (isRightPassword) {
			...
			String refreshToken = String.valueOf(refreshTokenSevice.createRefreshToken(foundRegistration.getEmail()));
			Integer expirationTimeInSeconds = tokenService.getExpireTime(authToken);
			CookieUtil.create(httpServletResponse, "SessionCookie", authToken, false, expirationTimeInSeconds,
			"localhost");
